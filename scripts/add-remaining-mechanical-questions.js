...
/**
 * Script to add remaining mechanical assessment questions to the existing test
 * This will add all the questions beyond the first 10 that were already created
 */
...
async function main() {
  console.log('🚀 Adding remaining Mechanical Assessment questions...\n');
  ...
  try {
    ...
    // All remaining mechanical assessment questions (questions 11-50)
    const remainingQuestions = [
      {
        promptText:
          'An autonomous wheeled robot is designed to navigate indoors using wheel odometry. The robot consistently registers a greater distance traveled than the actual distance. Assuming no wheel slip, what is the most likely cause of this systemic error?',
        category: 'OTHER',
        timerSeconds: 90,
        answerOptions: [
          'The encoder resolution is too low.',
          "The robot's programmed wheel diameter is slightly smaller than its actual effective diameter.",
          'The PID velocity controller has too high a proportional gain.',
          'The floor surface is too smooth.',
        ],
        correctAnswerIndex: 1,
        sectionTag: 'Robotics Navigation',
      },
      {
        promptText:
          'For a high-speed racing robot that must perform tight cornering on a flat, high-traction surface, what is the primary advantage of an Ackermann steering geometry over a differential (skid) steering system?',
        category: 'OTHER',
        timerSeconds: 90,
        answerOptions: [
          'Ackermann steering allows for a zero-radius turn.',
          'Ackermann steering minimizes tire scrub during turns, reducing power loss and improving cornering stability at high speed.',
          'Ackermann steering is mechanically simpler to implement than differential steering.',
          'Ackermann steering provides higher torque to the wheels.',
        ],
        correctAnswerIndex: 1,
        sectionTag: 'Vehicle Steering',
      },
      {
        promptText:
          'When designing a suspension system for a UGV that will traverse soft, sandy terrain, what is the most important characteristic of the tires and suspension?',
        category: 'OTHER',
        timerSeconds: 90,
        answerOptions: [
          'High spring rate and stiff damping to prevent bottoming out.',
          'Low ground pressure (wide, soft tires) and high suspension travel to maintain contact and float over the sand.',
          'Narrow, hard tires to cut through the sand to a firmer layer below.',
          'A very low center of gravity, even at the expense of ground clearance.',
        ],
        correctAnswerIndex: 1,
        sectionTag: 'Suspension Design',
      },
      {
        promptText:
          'A tracked robot is climbing a steep, low-friction incline. Where should the center of gravity be positioned for maximum climbing ability without slipping?',
        category: 'OTHER',
        timerSeconds: 90,
        answerOptions: [
          'As low and as far forward as possible.',
          'As low and as far backward as possible.',
          'As high and as central as possible.',
          'Perfectly centered in the middle of the track contact patch.',
        ],
      ...
      {
        promptText:
          'A mecanum wheel drivetrain is experiencing significant unintended lateral drift when commanded to move perfectly straight. The wheels, motors, and controllers are all identical and functioning correctly. What is the most likely mechanical root cause?',
        category: 'OTHER',
        timerSeconds: 90,
        answerOptions: [
          "The robot's weight distribution is slightly off-center.",
          'The four wheels are not mounted in a perfectly rectangular configuration, causing slight angular misalignment.',
          'The rollers on the mecanum wheels have too much backlash.',
          'The floor surface is not perfectly level.',
        ],
        correctAnswerIndex: 1,
        sectionTag: 'Mecanum Drives',
      },
      {
        promptText:
    ...
    console.log(
      `📝 Adding ${remainingQuestions.length} remaining questions...\n`
    );
    ...
    console.log(`\n🎉 Successfully added all remaining questions!`);
    ...
    // Group questions by section tag for summary
    const allQuestions = [...existingTest.questions, ...remainingQuestions];
    const sectionCounts = {};
    allQuestions.forEach((q) => {
      const tag = q.sectionTag || 'General';
